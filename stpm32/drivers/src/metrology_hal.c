...
static void Metro_HAL_STPM_SYN_reset_2_pulses(METRO_NB_Device_t in_Metro_Device_Id);
static void Metro_HAL_STPM_SYN_single_pulse(METRO_NB_Device_t in_Metro_Device_Id);
static void Metro_HAL_CSS_EXT_Device(METRO_NB_Device_t in_Metro_Device_Id,MET_GPIO_PIN_State_t in_Metro_enable);
static void Metro_HAL_EN_EXT_Device(METRO_NB_Device_t in_Metro_Device_Id,MET_GPIO_PIN_State_t in_Metro_enable);
static void Metro_HAL_SYN_EXT_Device(METRO_NB_Device_t in_Metro_Device_Id,MET_GPIO_PIN_State_t in_Metro_enable);
...
/**
  ...
  * @retval void
  */
int8_t Metrology_HAL_ReadBlock(METRO_NB_Device_t in_Metro_Device_Id, uint8_t Offset, uint8_t BlockNum, uint32_t * out_p_Buffer)
{
  ...
  if ( in_Metro_Device_Id >= EXT1)// modif
  {
    ...
    ret_size = Metro_HAL_Stpm_Read(in_Metro_Device_Id,(uint8_t*)&tmp_addr,BlockNum,out_p_Buffer);
...
/**
  ...
  * @retval void
  */
int8_t Metrology_HAL_WriteBlock(METRO_NB_Device_t in_Metro_Device_Id, uint8_t Offset, uint8_t BlockNum, uint32_t * in_p_Buffer)
{
  uint32_t tmp_addr = 0x0;
...
{
	metrology_platform_log(MET_LOG_INFO, "Device initialization: %d; %d", in_Metro_Device_Id, p_Metro_Device_Config[in_Metro_Device_Id].device);
  if(in_Metro_Device_Id >= EXT1 && p_Metro_Device_Config[in_Metro_Device_Id].device != Device_NONE)
  {

#ifdef UART_XFER_STPM3X // UART MODE
	  metrology_platform_log(MET_LOG_INFO, "Metro_HAL_usart_config 9600");
    ...
    Metro_HAL_CSS_EXT_Device(in_Metro_Device_Id,MET_GPIO_PIN_SET);
    ...
    Metro_HAL_WaitMicroSecond(1000); 
    Metro_HAL_CSS_EXT_Device(in_Metro_Device_Id,MET_GPIO_PIN_RESET);
    Metro_HAL_WaitMicroSecond(100); 
    Metro_HAL_CSS_EXT_Device(in_Metro_Device_Id,MET_GPIO_PIN_SET);
      
   /* Wait 20 ms */
   Metro_HAL_WaitMicroSecond(40000);
   
   /* Make one access ( first reg u32 reg) to ext chip to check if it is available */
   
   metrology_platform_log(MET_LOG_INFO, "Metro_HAL_Stpm_Read...");
   uint32_t addr = 0;
   if (Metro_HAL_Stpm_Read(in_Metro_Device_Id,(uint8_t*)&addr,1,&p_Metro_Device_Config[in_Metro_Device_Id].metro_stpm_reg.DSPCTRL1)<=0)
   {
	   metrology_platform_log(MET_LOG_WARN, "Device set to none");
...
void Metro_HAL_power_up_device(METRO_NB_Device_t in_Metro_Device_Id)
{
  if(in_Metro_Device_Id >= EXT1) 
  {
    ...
    Metro_HAL_WaitMicroSecond(1000); 
    Metro_HAL_EN_EXT_Device(in_Metro_Device_Id,MET_GPIO_PIN_RESET);
    Metro_HAL_WaitMicroSecond(1000); 
    Metro_HAL_EN_EXT_Device(in_Metro_Device_Id,MET_GPIO_PIN_SET);
    Metro_HAL_WaitMicroSecond(1000); 
  }
}

/**
  * @brief      This function reset device measurement registers
  * @param[in]   in_Metro_Device_Id (device ID), EXT1 to EXT4
  * @param[out]  none
  * @retval
  */
void Metro_HAL_reset_measures(METRO_NB_Device_t in_Metro_Device_Id)
{
  if(in_Metro_Device_Id >= EXT1) 
  {
    Metro_HAL_STPM_SYN_reset_2_pulses(in_Metro_Device_Id);
  }
}
...
/**
  ...
  * @retval
  */
int8_t Metro_HAL_Get_Data_device(METRO_NB_Device_t in_Metro_Device_Id)
{
  uint32_t tmp_addr = 0;
  ...
  if(in_Metro_Device_Id >= EXT1) 
  {
    ...
    uint32_t result = Metro_HAL_Stpm_Read(in_Metro_Device_Id,(uint8_t*)&tmp_addr,(METRO_STPM_DSP_DATA_REG_NB_BLOCKS + METRO_STPM_PH1_DATA_REG_NB_BLOCKS),&p_Metro_Device_Config[in_Metro_Device_Id].metro_stpm_reg.DSPEVENT1);
    if (result <= 0)
    {
    	return -1;
    }
    ...
    result = Metro_HAL_Stpm_Read(in_Metro_Device_Id,(uint8_t*)&tmp_addr,METRO_STPM_TOT_DATA_REG_NB_BLOCKS,&p_Metro_Device_Config[in_Metro_Device_Id].metro_stpm_reg.TOT_REG1);
	if (result <= 0)
	{
		return -1;
	}
    ...
    {
    tmp_addr = (uint32_t)((uint8_t*)&p_Metro_Device_Config[in_Metro_Device_Id].metro_stpm_reg.DSPEVENT1 - (uint8_t*)&p_Metro_Device_Config[in_Metro_Device_Id].metro_stpm_reg.DSPCTRL1)/2;
    uint32_t result = Metro_HAL_Stpm_Read(in_Metro_Device_Id,(uint8_t*)&tmp_addr,METRO_STPM_DATA_REG_NB_BLOCKS,&p_Metro_Device_Config[in_Metro_Device_Id].metro_stpm_reg.DSPEVENT1);
    if (result <= 0)
   	{
   		return -1;
   	}
...
void Metro_HAL_Set_Temperature_Compensation(METRO_NB_Device_t in_Metro_Device_Id,METRO_internal_Channel_t in_Metro_int_Channel, uint8_t in_Metro_TC_Value)
{
  ...
  if(in_Metro_Device_Id >= EXT1) 
  {
    ...
    uint32_t addr = 0;
    Metro_HAL_Stpm_write(in_Metro_Device_Id,(uint8_t*)&addr,2,&p_Metro_Device_Config[in_Metro_Device_Id].metro_stpm_reg.DSPCTRL1,STPM_WAIT);
...
{ 
  if(in_Metro_Device >= EXT1) 
  {
    ...
    uint32_t addr = 0;
    Metro_HAL_Stpm_write(in_Metro_Device,(uint8_t*)&addr,2,&p_Metro_Device_Config[in_Metro_Device].metro_stpm_reg.DSPCTRL1,STPM_WAIT);
  }
}
...
void Metro_HAL_Set_Coil_integrator(METRO_NB_Device_t in_Metro_Device_Id,METRO_internal_Channel_t in_Metro_int_Channel, METRO_CMD_Device_t in_Metro_CMD)
{
  if(in_Metro_Device_Id >= EXT1) 
  {
    if ( in_Metro_int_Channel == INT_CHANNEL_1)
    {
    	metrology_platform_log(MET_LOG_INFO, "Metro_HAL_Set_Coil_integrator channel1");
...
int32_t Metro_HAL_read_Momentary_Voltage(METRO_NB_Device_t in_Metro_Device_Id,METRO_internal_Channel_t in_Metro_int_Channel,METRO_Voltage_type_t in_Metro_V_type)
{
  ...
  if(in_Metro_Device_Id >= EXT1) 
  {
    ...
    if (in_Metro_int_Channel == INT_CHANNEL_1)
    {
      ...
      {
    	  uint32_t tmp_addr = 0x38;
    	  Metro_HAL_Stpm_Read(in_Metro_Device_Id,(uint8_t*)&tmp_addr,1,&p_Metro_Device_Config[in_Metro_Device_Id].metro_stpm_reg.DSP_REG6);
        /* get directly from RAM, be carrefull : latch should be made before to have coherents values inside DSP data reg */
      ...
      {
      	uint32_t tmp_addr = 0x30;
      	Metro_HAL_Stpm_Read(in_Metro_Device_Id,(uint8_t*)&tmp_addr,1,&p_Metro_Device_Config[in_Metro_Device_Id].metro_stpm_reg.DSP_REG2);
        /* get directly from RAM, be carrefull : latch should be made before to have coherents values inside DSP data reg */
    ...
    else if (in_Metro_int_Channel == INT_CHANNEL_2)/* is channel two */
    {
      if (in_Metro_V_type == V_FUND)
      {
    	  uint32_t tmp_addr = 0x3C;
    	  Metro_HAL_Stpm_Read(in_Metro_Device_Id,(uint8_t*)&tmp_addr,1,&p_Metro_Device_Config[in_Metro_Device_Id].metro_stpm_reg.DSP_REG8);
        /* get directly from RAM, be carrefull : latch should be made before to have coherents values inside DSP data reg */
        M_voltage = (p_Metro_Device_Config[in_Metro_Device_Id].metro_stpm_reg.DSP_REG8)&BIT_MASK_STPM_DATA_V;
      }
      else if (in_Metro_V_type == V_WIDE)
      {
    	  uint32_t tmp_addr = 0x34;
    	  Metro_HAL_Stpm_Read(in_Metro_Device_Id,(uint8_t*)&tmp_addr,8,&p_Metro_Device_Config[in_Metro_Device_Id].metro_stpm_reg.DSP_REG4);
        /* get directly from RAM, be carrefull : latch should be made before to have coherents values inside DSP data reg */
...
static void Metro_HAL_STPM_SYN_single_pulse(METRO_NB_Device_t in_Metro_Device_Id)
{
  ...
  Metro_HAL_SYN_EXT_Device(in_Metro_Device_Id,MET_GPIO_PIN_SET);
  /* reset SYNC pulse */
  Metro_HAL_SYN_EXT_Device(in_Metro_Device_Id,MET_GPIO_PIN_RESET);
...
/**
  * @brief  This function send 2 pulses on SYN signal to reset of measurement registers and counters
  * @param  METRO_NB_Device_t in_Metro_Device_Id
  * @retval None
  */
static void Metro_HAL_STPM_SYN_reset_2_pulses(METRO_NB_Device_t in_Metro_Device_Id)
{
  ...
  // Perform two consecutive pulses on EXT pin
  Metro_HAL_SYN_EXT_Device(in_Metro_Device_Id,MET_GPIO_PIN_SET);
  Metro_HAL_SYN_EXT_Device(in_Metro_Device_Id,MET_GPIO_PIN_RESET);
  Metro_HAL_WaitMicroSecond(50);
  Metro_HAL_SYN_EXT_Device(in_Metro_Device_Id,MET_GPIO_PIN_SET);
  Metro_HAL_WaitMicroSecond(50);
  Metro_HAL_SYN_EXT_Device(in_Metro_Device_Id,MET_GPIO_PIN_RESET);
  Metro_HAL_WaitMicroSecond(50);
...
/**
  * @brief  This function send 3 pulses on SYN signal to global reset STPM external chips
  * @param  METRO_NB_Device_t in_Metro_Device_Id
  * @retval None
  */
static void Metro_HAL_STPM_SYN_reset_3_pulses(METRO_NB_Device_t in_Metro_Device_Id)
{
	metrology_platform_log(MET_LOG_INFO, "Reset STPM chip...");
#ifdef UART_XFER_STPM3X /* UART MODE */   
...
static void Metro_HAL_WaitMicroSecond(uint32_t time)
{
	metrology_platform_wait_microseconds(time);
}
...
/**
  ...
  * @retval
  */
int32_t Metro_HAL_Stpm_write(METRO_NB_Device_t in_Metro_Device_Id,uint8_t *in_p_data,uint8_t nb_blocks,uint32_t * in_p_Buffer,uint8_t in_wait_stpm)
{
   uint32_t retSize = 0;
...
#ifdef UART_XFER_STPM3X /* UART MODE */
	/* Check CRC */
    uint8_t reversed[STPM3x_FRAME_LEN-1];
	for (i = 0; i < (STPM3x_FRAME_LEN - 1); i++) {
		 reversed[i] = Metro_HAL_byteReverse(p_Metro_Device_Config[in_Metro_Device_Id].STPM_com.pRxReadBuf[i]);
	}

	uint8_t crc = Metro_HAL_byteReverse(Metro_HAL_CalcCRC8(reversed));
	if (crc	!= p_Metro_Device_Config[in_Metro_Device_Id].STPM_com.pRxReadBuf[4]) {
		metrology_platform_log(MET_LOG_ERROR, "CRC check failed! crc: %d", crc);
	    /* Reset RX/TX Buffers */
	    memset(Metro_Com_TxBuf,0,METRO_BUFF_COM_MAXSIZE);
	    memset(Metro_Com_RxBuf,0,METRO_BUFF_COM_MAXSIZE);
	    return -1;
...
#ifdef SPI_XFER_STPM3X /* SPI MODE */
	uint8_t crc = Metro_HAL_CalcCRC8(p_Metro_Device_Config[in_Metro_Device_Id].STPM_com.pRxReadBuf);
	if (crc	!= p_Metro_Device_Config[in_Metro_Device_Id].STPM_com.pRxReadBuf[4]) {
		metrology_platform_log(MET_LOG_ERROR, "CRC check failed! crc: %d", crc);
		/* Reset RX/TX Buffers */
		memset(Metro_Com_TxBuf,0,METRO_BUFF_COM_MAXSIZE);
...
/**
  ...
  * @retval
  */
int32_t Metro_HAL_Stpm_Read(METRO_NB_Device_t in_Metro_Device_Id,uint8_t * in_p_data,uint8_t nb_blocks,uint32_t * out_p_read_data)
{
   uint32_t retSize = 0;
#ifdef UART_XFER_STPM3X /* UART MODE */  
   uint8_t CRC_on_reversed_buf;
...
static void Metro_HAL_CSS_EXT_Device(METRO_NB_Device_t in_Metro_Device_Id,MET_GPIO_PIN_State_t in_Metro_enable)
{
	metrology_platform_gpio_write(p_Metro_Device_Config[in_Metro_Device_Id].pin_handle, MET_PORT_CS, in_Metro_enable);
}
...
static void Metro_HAL_EN_EXT_Device(METRO_NB_Device_t in_Metro_Device_Id,MET_GPIO_PIN_State_t in_Metro_enable)
{
	metrology_platform_gpio_write(p_Metro_Device_Config[in_Metro_Device_Id].pin_handle, MET_PORT_EN, in_Metro_enable);
}
...
static void Metro_HAL_SYN_EXT_Device(METRO_NB_Device_t in_Metro_Device_Id,MET_GPIO_PIN_State_t in_Metro_enable)
{
	metrology_platform_gpio_write(p_Metro_Device_Config[in_Metro_Device_Id].pin_handle, MET_PORT_SYN, in_Metro_enable);
}
...
{
	metrology_platform_uart_config(p_Metro_Device_Config[in_Metro_Device_Id].uart_handle, in_baudrate);
}
...
//	HAL_UART_Receive(&huart[in_Metro_Device_Id], &data, 1, USART_TIMEOUT);
	metrology_platform_uart_transmit(p_Metro_Device_Config[in_Metro_Device_Id].uart_handle, &data, 1, 0);
	metrology_platform_uart_receive(p_Metro_Device_Config[in_Metro_Device_Id].uart_handle, &data, 1, USART_TIMEOUT);
	return data;
}
...
void Metro_HAL_Spi_config(METRO_NB_Device_t in_Metro_Device_Id)
{
  ...
  __HAL_SPI_ENABLE(&hspi[in_Metro_Device_Id]);*/
	metrology_platform_spi_config(p_Metro_Device_Config[in_Metro_Device_Id].spi_handle);
}
...
{  
  uint8_t data_in;
  metrology_platform_spi_transmit_receive(p_Metro_Device_Config[in_Metro_Device_Id].spi_handle, &data, &data_in, 1, SPI_TIMEOUT);
  return(data_in);
}
