...
* @retval 8 : 0 : Return OK, -1 Error
  */
int8_t Metro_Read_Block_From_Device ( METRO_NB_Device_t in_Metro_Device_Id, uint8_t in_Metro_Device_Offset_Adress, uint8_t in_Metro_Nb_of_32b_Reg, uint32_t *p_buffer )
{
  int8_t error = 0;
...
* @retval 8 : 0 : Return OK, -1 Error
  */
int8_t Metro_Write_Block_to_Device(METRO_NB_Device_t in_Metro_Device_Id, uint8_t in_Metro_Device_Offset_Adress, uint8_t in_Metro_Nb_of_32b_Reg, uint32_t *in_p_buffer )
{
  int8_t error = 0;
...
int8_t Metro_Get_Data_device(METRO_NB_Device_t in_Metro_Device)
{

  /* Get DSP data inside the Metrology chip requested  */
  if (in_Metro_Device < (NB_MAX_DEVICE))
  {
    return Metro_HAL_Get_Data_device(in_Metro_Device);
  }
...
/**
  * @brief  Latch measurements and reset measurement registers and counters
  * @param[in]   in_Metro_Device (Device ID ), HOST or EXT1 to EXT4
  * @param[out]  None
  * @retval     0 : Return OK, -1 Error
  */
uint8_t Metro_Set_Latch_and_reset(METRO_NB_Device_t in_Metro_Device)
{
  if (in_Metro_Device < (NB_MAX_DEVICE))
  {
	  Metro_HAL_reset_measures(in_Metro_Device);
  }
  return 0;
}
...
int32_t Metro_Read_Momentary_Voltage(METRO_Channel_t in_Metro_Channel, METRO_Voltage_type_t in_Metro_V_type)
{
  ...
  raw_M_Voltage = (raw_M_Voltage & 0x007FFFFF) - (0x800000 * (raw_M_Voltage >> 23));
...
int32_t Metro_Read_Momentary_Current(METRO_Channel_t in_Metro_Channel, METRO_Current_type_t in_Metro_C_type)
{
  ...
  raw_M_Current = (raw_M_Current & 0x007FFFFF) - (0x800000 * (raw_M_Current >> 23));
...
int32_t Metro_Read_PHI(METRO_Channel_t in_Metro_Channel)
{
 ...
 /* phase angle = Calc_phi *f*360/Fclk */
   Calc_phi = raw_phi == 0 ? 0 : (int64_t) ((raw_phi * 2880) / period);
...
void Metro_Set_Uart_Handle(METRO_NB_Device_t in_Metro_Device,  struct uart_handle_data_tag *uart_handle)
{
	if (in_Metro_Device < (NB_MAX_DEVICE))
	{
		Tab_METRO_internal_Devices_Config[in_Metro_Device].uart_handle = uart_handle;
	}
}

void Metro_Set_Spi_Handle(METRO_NB_Device_t in_Metro_Device, struct spi_handle_data_tag *spi_handle)
{
	if (in_Metro_Device < (NB_MAX_DEVICE))
	{
		Tab_METRO_internal_Devices_Config[in_Metro_Device].spi_handle = spi_handle;
	}
}

void Metro_Set_Pin_Handle(METRO_NB_Device_t in_Metro_Device, struct pin_handle_data_tag *pin_handle)
{
	if (in_Metro_Device < (NB_MAX_DEVICE))
	{
		Tab_METRO_internal_Devices_Config[in_Metro_Device].pin_handle = pin_handle;
	}
}
